...
class SpeedManager {
  ...
  Future<int>? getHighScore(String uid) async {
    ...
    try {
      snapshot = userList.doc(uid).collection("Game 3").doc("High Score");
  ...
  Future<int>? getUserLevel(String uid) async {
    ...
    try {
      snapshot = userList.doc(uid).collection("Game 3").doc("Level");
      progress = await snapshot.get();
  ...
  Future<int>? getUserPlayTime(String uid) async {
    ...
    try {
      snapshot = userList.doc(uid).collection("Game 3").doc("Times Played");
      progress = await snapshot.get();
  ...
  Future<bool>? getUnlockStatus(String uid) async {
    ...
    try {
      snapshot = userList.doc(uid).collection("Game 3").doc("Unlocked");
      progress = await snapshot.get();
  ...
  Future<void> levelUp(
    ...
    SpeedManager _speedManager = SpeedManager();

    int _userLevel = await _speedManager.getUserLevel(uid)!;
    int _userExp = await _speedManager.getUserExp(uid)!;
